...
| `target` | `object` | 生成的演绎版的多部分预签名URL上传信息。 这适用于具有此[多部分上传行为](https://jackrabbit.apache.org/oak/docs/apidocs/org/apache/jackrabbit/api/binary/BinaryUpload.html)的[AEM/Oak直接二进制上传](https://jackrabbit.apache.org/oak/docs/features/direct-binary-access.html)。<br>字段:<ul><li>`urls`:字符串数组，每个预签名的部件URL对应一个</li><li>`minPartSize`:用于一部分的最小大小= url</li><li>`maxPartSize`:一部分使用的最大大小= url</li></ul> | `{ "urls": [ "https://part1...", "https://part2..." ], "minPartSize": 10000, "maxPartSize": 100000 }` |
